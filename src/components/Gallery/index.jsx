...
function PhotoCard({ photo, onOpen }) {
  ...
  const heightClass = photo.h === "tall" ? "h-72" : photo.h === "short" ? "h-44" : "h-56";

  return (
    <div
      className="relative rounded-xl overflow-hidden cursor-zoom-in group mb-3"
    ...
    >
      <div className={`${heightClass} w-full relative`}>
        ...
        {!loaded && (
          <div
            ...
            style={{ background: photo.color + "55" }}
          />
        )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 pointer-events-none z-20">
        <p className="text-white text-xs font-semibold drop-shadow">{photo.author}</p>
...
// ─────────────────────────────────────────────────────────────
export default function ImageGallery({ photos = defaultPhotos, filters = defaultFilters }) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 sm:py-4 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
          <span className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight flex-shrink-0 text-left">Gallery</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-gray-400 gap-3">
            ...
            <p className="text-base text-center">No photos found for "<strong>{search || activeFilter}</strong>"</p>
            <button
