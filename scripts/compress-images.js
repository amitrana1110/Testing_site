    ...
    for (const file of files) {
      ...
      if (ext === '.webp' || ext === '.png' || ext === '.jpg' || ext === '.jpeg') {
        try {
          ...
          if (ext === '.webp') {
            compressedBuffer = await pipeline.webp({ quality: 75, effort: 4 }).toBuffer();
          } else if (ext === '.png') {
            compressedBuffer = await pipeline.png({ compressionLevel: 8 }).toBuffer();
          } else {
            compressedBuffer = await pipeline.jpeg({ quality: 75, progressive: true }).toBuffer();
          }
          
          // Only overwrite if we actually reduced the size
          if (compressedBuffer.length < originalSize) {
            fs.writeFileSync(filePath, compressedBuffer);
            console.log(`✔ Compressed ${file}: ${(originalSize / 1024).toFixed(1)} KB → ${(compressedBuffer.length / 1024).toFixed(1)} KB`);
          } else {
